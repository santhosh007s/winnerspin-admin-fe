...
//         list.map((p) => ({
//           _id: String(p?._id ?? ""),
//           userid: String((p as any)?.userid ?? (p as any)?.username ?? ""), // backend sometimes sends username instead of userid
//           username: String(p?.username ?? ""),
//           email: String(p?.email ?? ""),
//           mobNo: String(p?.mobNo ?? ""),
//           status,
...
//       setApprovedPromoters(
//         normalize((response as any).approvedPromoters ?? [], "approved", true)
//       );
//       setNonApprovedPromoters(
//         normalize(
//           (response as any).nonApprovedPromoters ?? [],
//           "unapproved",
//           true
//         )
//       );
//       setInactivePromoters(
//         normalize((response as any).inactivePromoters ?? [], "inactive", false)
//       );
//       setAllInactivePromoters(
//         normalize(
//           (response as any).allInactivePromoters ?? [],
//           "inactive",
//           true
//         )
//       );
...
//   // Merge ALL for export, unique by _id (latest occurrence wins)
...
//       // Construct a plain array of rows
...
//         Balance: p.balance,
//         CustomersCount: p.customers?.length ?? 0,
//         Customers: (p.customers ?? []).join(", "),
//       }));
...
//       const filename = `promoters_${stamp.getFullYear()}-${pad(
//         stamp.getMonth() + 1
